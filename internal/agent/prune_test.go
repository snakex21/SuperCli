...
// prune first, summary only if the estimate STILL crosses 80% of the
// window. When prune reclaims enough, the summarizer must not be
// called.
func TestPrune_RunsBeforeSummaryFallback(t *testing.T) {
	...
	if l.EstimateVisibleTokens() > 8000 {
		t.Errorf("estimate still above 80%% window: %d", l.EstimateVisibleTokens())
	}
}
