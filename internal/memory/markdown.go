...
// atomicWrite writes data to a temp file in the same directory
// and renames over the target. Falls back to plain WriteFile
// when rename fails (e.g. cross-device on some filesystems).
func atomicWrite(path string, data []byte) error {
	...
	if err := os.Rename(tmpName, path); err != nil {
		...
		if err2 := os.WriteFile(path, data, 0o644); err2 != nil {
			return fmt.Errorf("memory.atomicWrite: rename failed (%v) and fallback failed (%v)", err, err2)
		}
	}
	return nil
}
