...
// BuildAddon validates user-selected files through the same sandbox
...
func BuildAddon(home string, paths []string) (string, error) {
	if len(paths) == 0 {
		return "", nil
	}
	if len(paths) > MaxFiles {
		return "", fmt.Errorf("too many files: %d (maximum %d)", len(paths), MaxFiles)
	}
...
// StagePicked validates an explicit desktop file-picker selection.
...
func StagePicked(home string, paths []string) ([]string, error) {
	if len(paths) > MaxFiles {
		return nil, fmt.Errorf("too many files: %d (maximum %d)", len(paths), MaxFiles)
	}
	absHome, err := filepath.Abs(home)
	if err != nil {
	...
	for _, raw := range paths {
		...
		if info.Size() > MaxFileBytes {
			return fail(fmt.Errorf("%q is too large: %d bytes (maximum %d)", filepath.Base(raw), info.Size(), MaxFileBytes))
		}
		total += info.Size()
		if total > MaxTotalBytes {
			return fail(fmt.Errorf("attachments are too large in total: %d bytes (maximum %d)", total, MaxTotalBytes))
		}
		...
		targetRoot, err := os.MkdirTemp(root, "file-")
		if err != nil {
			_ = in.Close()
			return fail(fmt.Errorf("create attachment stage: %w", err))
		}
		target := filepath.Join(targetRoot, SafeName(filepath.Base(source)))
		...
		if written > MaxFileBytes {
			copyErr = fmt.Errorf("file grew beyond the %d-byte limit while copying", MaxFileBytes)
		}
