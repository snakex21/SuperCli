...
func TestSettings_CompactModelPersists(t *testing.T) {
	m := newSettingsModel(t, "")
	m = cursorForKey(m, "compact_model")
	mm, _ := m.settingsEnter()
	m = mm.(Model)
	if !m.menu.editing {
		t.Fatal("expected editing mode after Enter on compact_model")
	}
	m = typeRunes(m, "cheap/qwen")
	mm, _ = m.settingsEditKey(tea.KeyMsg{Type: tea.KeyEnter})
	m = mm.(Model)
	if cfg := loadCfg(t, m); cfg.CompactModel != "cheap/qwen" {
		t.Fatalf("compact_model = %q, want cheap/qwen", cfg.CompactModel)
	}
}
