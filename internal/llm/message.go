...
// ImageRef references an image either by URL (http(s) or data: URI)
// or by raw base64 data + MIME type. URL is preferred for HTTP; Data
// is preferred for images the agent just read off disk.
type ImageRef struct {
	URL       string // http(s) URL or "data:<mediatype>;base64,<data>" URI
	MediaType string // "image/png" | "image/jpeg" | "image/webp" | "image/gif"
	Data      string // raw base64 (no prefix); used when URL is empty
}

// AsDataURI returns a data: URI suitable for OpenAI's image_url.url
// field. If URL is already a data: URI it is returned as-is. If only
// Data is set, it is composed with MediaType.
...
func (p ContentPart) Validate() error {
	...
	case PartTypeImage:
		...
		if p.Image.URL == "" && p.Image.Data == "" {
			return fmt.Errorf("image part: URL and Data both empty")
		}
		if p.Image.URL == "" && p.Image.MediaType == "" {
			return fmt.Errorf("image part: MediaType required when Data is set")
		}
	case "":
