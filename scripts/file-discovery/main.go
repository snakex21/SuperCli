// Controlled, read-only experiment for filename discovery.
...
func main() {
	...
	root := filepath.Join(filepath.Dir(outfile), filepath.Base(outfile)+".repo")
	sources := []string{
		"cmd/server/main.go", "src/auth/login.go", "src/auth/token.go",
		"src/billing/invoice.go", "src/billing/receipt.go", "src/storage/cache.go",
		"src/storage/sqlite.go", "src/sync/pull.go", "src/sync/push.go", "src/storage/placeholder.go",
	}
	...
	for _, name := range fixture {
		...
		body := strings.Repeat("// license header\n", 12) + "package example\nfunc Example() {}\n"
		if strings.HasSuffix(name, "placeholder.go") {
			body = ""
		}
	...
	defer cancel()
	ctx = llm.WithOpenCodeSession(ctx, "file-discovery-"+filepath.Base(outfile))
	...
	answer := res.Text
	if start := strings.Index(answer, "<result>"); start >= 0 {
		answer = answer[start+len("<result>"):]
		if end := strings.Index(answer, "</result>"); end >= 0 {
			answer = answer[:end]
		}
	}
