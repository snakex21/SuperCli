// Controlled continuation after a bounded read with an oversized range.
package main
...
func main() {
	...
	var results []outcome
	for _, name := range []string{"short-file", "unread-tail", "valid-control"} {
...
func evaluate(root, name string, backend llm.Provider, caps *llm.CapabilityRegistry) outcome {
	...
	raw, _ := json.Marshal(map[string]any{"file": "limits.txt", "from": from, "to": to})
	...
	loop.Messages = append(loop.Messages,
		llm.Message{Role: llm.RoleUser, Content: "Read the requested range of limits.txt and find RetryLimit."},
	...
	defer cancel()
	ctx = llm.WithOpenCodeSession(ctx, "range-cap-"+filepath.Base(filepath.Dir(root))+"-"+name)
	start := time.Now()
	events, err := loop.Run(ctx, "Znajdź wartość RetryLimit w limits.txt. Odpowiedz jedną linią: RetryLimit=<wartość>.")
	if err != nil {
		r.Error = err.Error()
	} else {
	...
	r.Correct = r.Error == "" && strings.TrimSpace(r.Answer) == "RetryLimit=6842"
	return r
}
func must(err error) {
	if err != nil {
